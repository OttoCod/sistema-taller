...
export function VentaDetalleDialog({ ventaId, onOpenChange }: Props) {
  ...
  const comprobantesQuery = useQuery({
    queryKey: ["comprobantes", "porVenta", ventaId],
    queryFn: () => listarComprobantesPorVenta(ventaId as number),
    enabled: open,
  });
  ...
  return (
    <>
              ...
              <>
                ...
                {venta.estado === "confirmada" && (
                  <div className="mt-4 flex items-center gap-3 border-t border-line pt-4">
                    <button
                      type="button"
                      onClick={() => setComprobanteAbierto("ticket")}
                    ...
                    <button
                      type="button"
                      onClick={() => setComprobanteAbierto("a4")}
                      className="rounded-md border border-line px-3 py-1.5 text-sm text-ink hover:bg-surface-2"
                    >
                      Imprimir A4
                    </button>
                    {comprobantes.length > 0 && (
                      <span className="text-xs text-ink-muted">
                        Ya generado: {comprobantes.map((c) => c.numero).join(", ")}
                      </span>
                    )}
                  </div>
                )}
      ...
      <ComprobanteDialog
        ...
        tipo={comprobanteAbierto ?? "ticket"}
        onOpenChange={(abierto) => {
          if (!abierto) {
            setComprobanteAbierto(null);
            queryClient.invalidateQueries({ queryKey: ["comprobantes", "porVenta", ventaId] });
          }
        }}
      />
    </>
